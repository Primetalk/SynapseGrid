...
trait Signals {
  ...
  trait SignalOnContacts[Contacts <: UniSet] extends SignalOnContacts0 {
  ...
  trait SignalOnContactsOps[S[_<:UniSet]<:SignalOnContacts0] {
...
//  }
//
  sealed trait MySignalOnContacts[Contacts <: UniSet] extends SignalOnContacts[Contacts]
...
//    (s: SignalOnContacts[A]) => ??? // new MySignal[s.C, B](s.contact, s.data, inferEBelongsToBIfEBelongsToASubset[s.C, A, B](s.contactIsInContacts, aIsSubsetOfB))
  ...
  def lift[In <: Contact, Out <: Contact]
    ...
    : signalOnContactsOps.Set[Singleton[In]] => Iterable[signalOnContactsOps.Set[Singleton[Out]]] =
  {
    signalOnContactIn =>
  ...
  def liftIterable[In <: Contact, Out <: Contact]
    ...
    (implicit signalOnContactsOps: SignalOnContactsOps[SignalOnContacts])
    : SignalOnContacts[Singleton[In]] => Iterable[SignalOnContacts[Singleton[Out]]] =
  {
    signalOnContactIn =>
      ??? //signalOnContactsOps.getIterable(signalOnContactIn, in)
  ...
  def trivialLift[C <: Contact]
    (c: C)
    (implicit signalOnContactsOps: SignalOnContactsOps[SignalOnContacts])
    : SignalOnContacts[Singleton[C]] => Iterable[SignalOnContacts[Singleton[C]]] =
    Iterable.single
